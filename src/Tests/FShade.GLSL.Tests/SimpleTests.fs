...
let ``New Intrinsics``() =
    ...
    let shader (v : Vertex) =
        vertex {
            let _ = VecFun.Min(V2f.Zero, V2f(v.c.XY))
            let _ = VecFun.Min(V2d.Zero, v.c.XY)
            let _ = VecFun.Min(V2i.Zero, V2i(v.c.XY))
            let _ = VecFun.Min(V3f.Zero, V3f(v.c.XYZ))
            let _ = VecFun.Min(V3d.Zero, v.c.XYZ)
            let _ = VecFun.Min(V3i.Zero, V3i(v.c.XYZ))
            let _ = VecFun.Min(V4f.Zero, V4f(v.c))
            let _ = VecFun.Min(V4d.Zero, v.c)
            let _ = VecFun.Min(V4i.Zero, V4i(v.c))
            let _ = VecFun.Max(V2f.Zero, V2f(v.c.XY))
            let _ = VecFun.Max(V2d.Zero, v.c.XY)
            let _ = VecFun.Max(V2i.Zero, V2i(v.c.XY))
            let _ = VecFun.Max(V3f.Zero, V3f(v.c.XYZ))
            let _ = VecFun.Max(V3d.Zero, v.c.XYZ)
            let _ = VecFun.Max(V3i.Zero, V3i(v.c.XYZ))
            let _ = VecFun.Max(V4f.Zero, V4f(v.c))
            let _ = VecFun.Max(V4d.Zero, v.c)
            let _ = VecFun.Max(V4i.Zero, V4i(v.c))
            let _ = VecFun.Clamp(V2f(v.c.XY), V2f.Zero, V2f.One)
            let _ = VecFun.Clamp(V2f(v.c.XY), 0.0f, 1.0f)
            let _ = VecFun.Clamp(v.c.XY, V2d.Zero, V2d.One)
            let _ = VecFun.Clamp(v.c.XY, 0.0, 1.0)
            let _ = VecFun.Clamp(V3f(v.c.XYZ), V3f.Zero, V3f.One)
            let _ = VecFun.Clamp(V3f(v.c.XYZ), 0.0f, 1.0f)
            let _ = VecFun.Clamp(v.c.XYZ, V3d.Zero, V3d.One)
            let _ = VecFun.Clamp(v.c.XYZ, 0.0, 1.0)
            let _ = VecFun.Clamp(V4f(v.c), V4f.Zero, V4f.One)
            let _ = VecFun.Clamp(V4f(v.c), 0.0f, 1.0f)
            let _ = VecFun.Clamp(v.c, V4d.Zero, V4d.One)
            let _ = VecFun.Clamp(v.c, 0.0, 1.0)
            let _ = Vec.lerp V2d.Zero V2d.One v.c.X
            let _ = Fun.Lerp(float32 v.c.X, 0uy, 1uy)
            let _ = Fun.Lerp(float32 v.c.X, 0us, 1us)
            let _ = Fun.Lerp(float32 v.c.X, 0s, 1s)
            let _ = Fun.Lerp(float32 v.c.X, 0u, 1u)
            let _ = Fun.Lerp(float32 v.c.X, 0, 1)
            let _ = Fun.Lerp(float32 v.c.X, 0.0f, 1.0f)
            let _ = Fun.Lerp(float32 v.c.X, 0.0, 1.0)
            let _ = Fun.Lerp(float v.c.X, 0uy, 1uy)
            let _ = Fun.Lerp(float v.c.X, 0us, 1us)
            let _ = Fun.Lerp(float v.c.X, 0s, 1s)
            let _ = Fun.Lerp(float v.c.X, 0u, 1u)
            let _ = Fun.Lerp(float v.c.X, 0, 1)
            let _ = Fun.Lerp(float v.c.X, 0.0f, 1.0f)
            let _ = Fun.Lerp(float v.c.X, 0.0, 1.0)
